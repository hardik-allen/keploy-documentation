...
Testing these applications traditionally requires:

- **1.Setting up test databases** — Provisioning PostgreSQL/MySQL instances, running migrations, managing schemas
- **2.Writing complex SQL fixtures** — Creating test data that mirrors production scenarios
- **3.Mocking database connections** — Using libraries like `go-sqlmock` or writing custom mock drivers
- **4.Maintaining test data** — Keeping fixtures in sync as your schema evolves, ensuring data consistency across test runs

**Keploy solves this** by recording your actual database interactions during normal API usage. No mocks, no fixtures—just real behavior captured and replayed as tests.
...
- **Basic Echo knowledge** — familiarity with Echo's router, handlers, and middleware
- **SQL database experience** — understanding of SQL queries and database connections
...
```
echo-sql-app/
├── main.go              # Your Echo application entry point
├── handlers/            # HTTP handlers
├── models/             # Data models
├── database/           # Database connection and migrations
├── docker-compose.yml   # Database service configuration
├── keploy/             # Keploy test cases (generated)
...
1. **HTTP Requests** — All incoming API calls to your Echo routes
2. **HTTP Responses** — Echo's responses, including status codes and JSON payloads
3. **SQL Queries** — Every database query executed during request handling
4. **Query Results** — The data returned from your SQL database
5. **Transaction Boundaries** — Database transactions and their outcomes

<WhyBox title="Why this matters">
Unlike unit tests that mock the database, Keploy captures real SQL interactions. This means your tests verify actual database behavior, not assumptions about how your queries work.
</WhyBox>
...
**Don't have Keploy installed yet?** Before running this sample, make sure Keploy is installed on your system. 👉 [Go to Installation Guide](https://keploy.io/docs/server/installation/)
</Callout>

<h2 id="docker-compose-method">Using Docker Compose 🐳</h2>

This method runs both your application and PostgreSQL in Docker containers, making it easy to get started.

<h3 id="echo-step-1">Step 1: Clone the Sample URL Shortener App 🧪</h3>
...
![Docker Compose Output](/tutorial-images/step2-docker-compose-start.png)
*Docker Compose starting PostgreSQL and building the application*

<WhyBox title="Why Docker Compose?">
It manages both your application and database dependencies, ensuring they're properly networked and can communicate with each other.
</WhyBox>
...
<WhyBox title="Why this matters for Go developers">
Traditional testing requires you to either mock the database (losing real SQL validation) or set up test databases manually. Keploy eliminates this choice—you get real database interactions captured automatically, ensuring your tests verify actual SQL behavior, not assumptions about how your queries work.
</WhyBox>
...
![Docker Build Output](/tutorial-images/step3-docker-build-echo-app.png)
*Terminal showing successful Docker build of echo-app:1.0*
...
![PostgreSQL and Echo Application Logs](/tutorial-images/step3-postgres-echo-running.png)
*Terminal showing PostgreSQL database ready and Echo server started on port 8082*

<WhyBox title="Why this matters for Go developers">
Docker provides a clean, reproducible environment. Your Echo app runs exactly the same way in the container as it would in production, and Keploy can observe all interactions without interfering with your local development setup. This means you can test with confidence that what works in the container will work in production.
</WhyBox>
...
<WhyBox title="Why eBPF?">
eBPF allows Keploy to intercept network traffic at the kernel level, capturing all interactions without modifying your application code.
</WhyBox>
...
![Keploy Recording Test Cases](/tutorial-images/step4-keploy-capture-testcases.png)
*Keploy terminal logs showing successful capture of test cases using eBPF*

<WhyBox title="Why this is revolutionary for Go developers">
...
![Postman API Request](/tutorial-images/step5-postman-create-url.png)
*Postman showing POST request to create a shortened URL and the JSON response*
...
keploy/
├── tests/
│   └── test-*.yaml    # Your captured test cases
└── mocks/             # PostgreSQL operation mocks
```
...
<WhyBox title="Why this approach is powerful for Go developers">
...
![Keploy Test Results](/tutorial-images/step6-keploy-test-summary.png)
*Keploy test run summary showing all tests passed successfully*

**What this means**: So no need to setup fake database/APIs like Postgres or write mocks for them. Keploy automatically mocks them, and the application thinks it's talking to Postgres 😄
...
<WhyBox title="Why this is transformative for Go developers">
...
- **Deterministic Tests**: 
  ...
  - Keploy's mocks are deterministic—same input always produces same output
...
<h2 id="local-linux-method">Running App Locally on Linux/WSL 🐧</h2>
...
<h3 id="local-step-1">Step 1: Clone the Sample App 🧪</h3>
...
<WhyBox title="Why `sudo`?">
Keploy uses eBPF which requires elevated privileges to intercept network traffic at the kernel level.
</WhyBox>
...
<WhyBox title="Why this matters for Go developers">
Running locally gives you the ability to:
...
**What this accomplishes**: So no need to setup dependencies like PostgreSQL locally or write mocks for your testing. The application thinks it's talking to PostgreSQL 😄
...
<h2 id="wrapping-up">Wrapping it up 🎉</h2>
...
<h2 id="echo-best-practices">Best Practices to keep in mind while recording testcases using Keploy</h2>
...
- ✅ `database/sql` with `pq` (PostgreSQL)
- ✅ `database/sql` with `go-sql-driver/mysql` (MySQL)
- ✅ `database/sql` with `mattn/go-sqlite3` (SQLite)
- ✅ Most ORMs built on top of `database/sql` (GORM, sqlx, etc.)
...
<WhyBox title="Why this happens">
When you create a resource, PostgreSQL generates:
...
- ✅ Echo (what we're using in this tutorial)
- ✅ Gin
- ✅ Fiber
- ✅ Chi
- ✅ Gorilla Mux
- ✅ Standard `net/http`
- ✅ Any framework that uses Go's standard HTTP interfaces

<WhyBox title="Why it works universally">
Keploy uses eBPF to intercept HTTP traffic at the kernel level, before it even reaches your framework. This means:
...
**A:** Updating test cases is simple—just re-record them!
...
<WhyBox title="Why this is powerful">
Traditional integration tests in CI/CD require:
